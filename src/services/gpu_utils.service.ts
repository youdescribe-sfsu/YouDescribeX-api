...
class GpuUtilsService {
  ...
  public async notifyAiDescriptions(youtube_id: string, audio_description_id: string, ydx_app_host: string, user_ids: string[]) {
    const videoData = await MongoVideosModel.findOne({ youtube_id: youtube_id, youtube_status: 'available' });
    ...
    const subject = `Requested Audio Description for ${videoData.title} ready`;
