...
class UserService {
  ...
  public async createNewUserAudioDescription(newUserAudioDescription: CreateUserAudioDescription) {
    ...
    if (CURRENT_DATABASE == 'mongodb') {
      ...
      const checkIfAudioDescriptionExists = await PostGres_Audio_Descriptions.findOne({ where: { VideoVideoId: videoIdStatus.video_id, UserUserId: userId } });
      ...
      const checkIfAIDescriptionsExists = await PostGres_Audio_Descriptions.findOne({
        where: { VideoVideoId: videoIdStatus.video_id, UserUserId: aiUserId }, include: [{
